...
export default function LawUpdatesPage() {
  ...
  useEffect(() => {
    console.log('[Law Updates] Fetching...');
    fetch('/api/law-updates')
      ...
      .then((data) => {
        console.log('[Law Updates] Fetched:', data);
        setArticles(data.articles || []);
      })
      .catch((err) => {
        console.error('[Law Updates] Error fetching:', err);
        setArticles([]);
      })
      .finally(() => {
        setLoading(false);
        console.log('[Law Updates] Loading finished');
      });
  }, []);
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {articles.map((article, idx) => (
          ...
          >
            {article.urlToImage && (
              <img src={article.urlToImage} alt="Law Update" className="w-full h-40 object-cover" />
            )}
            <div className="p-4">
              <h2 className="text-md font-bold text-yellow-800 line-clamp-2 min-h-[3rem]">{article.title}</h2>
      ...
      >
        <div className="bg-white max-w-lg w-full rounded-xl shadow-lg p-6 space-y-4 overflow-y-auto max-h-[80vh]">
          ...
          <h2 className="text-xl font-bold text-yellow-800">{selectedArticle?.title}</h2>
